...
import { ProductFormProps, SizeQuantity } from "../../types/ProductTypes";
...
const ProductForm = ({
    ...
    const toggleSize = (sizeValue: string) => {
        const selected =
            formik.values.sizeQuantities.some(
        ...
        if (selected) {
            formik.setFieldValue(
                "sizeQuantities",
                formik.values.sizeQuantities.filter(
                    (item: any) => item.size !== sizeValue
                )
            );
        } else {
            ...
            <Grid container spacing={2}>
                    ...
                    <div className="border rounded-xl p-5 bg-gray-50">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="text-sm text-gray-500">
                                ...
                                {" "}
                                {
                                    formik.values.sizeQuantities.reduce(
                                        (sum: number, item: SizeQuantity) => sum + item.quantity,
                                        0
                                    )
                                }
                            </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {sizes.map((size) => {
                                const selected =
                                    formik.values.sizeQuantities.some(
                                        (item: SizeQuantity) => item.size === size.value
                                    );
                                    ...
                                    >
                                        ...
                                        {selected && (
                                            <TextField
                                                ...
                                                value={
                                                    formik.values.sizeQuantities.find(
                                                        (item: SizeQuantity) => item.size === size.value
                                                    )?.quantity || 0
                                                }
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    const updated =
                                                        formik.values.sizeQuantities.map(
                                                            (item: SizeQuantity) =>
                                                                item.size === size.value
                                                                    ? {
                                                                        ...item,
                ...
                <Grid size={{ xs: 12, md: 4, lg: 3 }}>
                    ...
                    >
                        ...
                        >
                            {
                                mainLevelCategory.map((item) => <MenuItem  key={item.categoryId} value={item.categoryId}>
                                    {item.name}
                                </MenuItem>)
                            }
                        </Select>
